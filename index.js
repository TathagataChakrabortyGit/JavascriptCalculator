...
const [input, setInput] = React.useState("0");
    ...
    if (!calculatorData.length) {
      ...
      if (value === 0 && (calculatorData === "0" || input === "0")) {
        ...
        const isLastChatOperator =
          lastChat === "*" || functions.includes(lastChat);
  ...
  const dotOperator = () => {
    ...
    if (!calculatorData.length) {
      ...
      if (lastChat === "*" || functions.includes(lastChat)) {
        setInput("0.");
  ...
  const handlefunctions = (value) => {
    if (calculatorData.length) {
      ...
      const beforeLastChatIsOperator =
        functions.includes(beforeLastChat) || beforeLastChat === "*";

      const lastChat = calculatorData.charAt(calculatorData.length - 1);
      
      const lastChatIsOperator = functions.includes(lastChat) || lastChat === "*";
  ...
  const handleInput = (value) => {
    ...
    const operator = functions.find((op) => op === value);
